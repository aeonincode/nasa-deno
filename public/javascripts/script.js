...
function listUpcoming() {
  ...
  launches
    ...
    .forEach((launch) => {
      ...
      upcomingList.innerHTML +=
        `<div class="list-item"><a class="delete" onclick="abortLaunch(${launch.flightNumber})">
✖
</a> ${flightNumber} <span class="silver">${launchDate}</span> ${mission} <span class="silver">${rocket}</span> <span class="gold">${target}</span></div>`;
    });
}
...
//       const target = launch.target ?? "";
//       upcomingList.innerHTML +=
//         `<div class="list-item"><a class="delete" onclick="abortLaunch(${launch.flightNumber})">âœ–</a> ${flightNumber} <span class="silver">${launchDate}</span> ${mission} <span class="silver">${rocket}</span> <span class="gold">${target}</span></div>`;
//     });
// }
...
//     .forEach((launch) => {
//       const success = launch.success
//         ? `<span class="success">â–ˆ</span>`
//         : `<span class="failure">â–ˆ</span>`;
//       const launchDate = new Date(launch.launchDate * 1000).toDateString();
//       const flightNumber = String(launch.flightNumber).padEnd(3);
//       const mission = launch.mission.slice(0, 25).padEnd(25);
//       const rocket = launch.rocket.padEnd(22);
